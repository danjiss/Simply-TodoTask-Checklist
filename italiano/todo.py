    ...
    def open_task(self):
        ...
        if link:
            try:
                ...
                print(f"Errore nell'aprire {link}: {e}")
...
class WelcomeScreen(QWidget):
    ...
    def setup_ui(self):
        ...
        welcome_text = QLabel(
            "Benvenuto in Simply TodoTask!\n\n"
            "Per iniziare, carica una lista esistente\n"
        ...
        load_btn = ModernButton("📂 Carica lista TodoTask", "#2196F3", height=50)
        personalize_btn = ModernButton("🎯 Crea / Personalizza Todo", "#4CAF50", height=50)
        ...
        author_label = QLabel("Creato con ♥ da Daniele Borghi")
        author_label.setStyleSheet("font-size: 11px; color: #2E86AB;")
        author_label.setAlignment(Qt.AlignmentFlag.AlignCenter)

        github_label = QLabel("Visita il mio repository su GITHUB per maggiori informazioni!")
...
class SaveDialog(QDialog):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Salva lista")
    ...
    def setup_ui(self):
        ...
        message = QLabel("Come vuoi salvare la lista?")
        ...
        save_btn = ModernButton("💾 Salva", "#2196F3", height=45)
        save_as_btn = ModernButton("📁 Salva come nuovo", "#4CAF50", height=45)
        cancel_btn = ModernButton("❌ Annulla", "#757575", height=45)
...
class ResetDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        message = QLabel("Cosa vuoi resettare?")
        message.setStyleSheet("""
            QLabel {
                font-size: 14px;
                font-weight: bold;
        ...
        reset_checks_btn = ModernButton("🔄 Reset Check", "#2196F3", height=45)
...
class CustomizeDialog(QDialog):
    ...
    def setup_ui(self):
        self.setWindowTitle("Personalizza To-Do")
        ...
        title_label = QLabel("🎯 Personalizza To-Do List")
        title_label.setStyleSheet("""
            QLabel {
                font-size: 22px;
                font-weight: bold;
        ...
        if self.current_file_path:
            file_info = QLabel(f"📄 File corrente: {self.current_file_path}")
        ...
        load_btn = ModernButton("📂 Carica Lista", "#FF9800", height=45)
        reset_btn = ModernButton("🔄 Reset To-Do", "#f44336", height=45)
        cancel_btn = ModernButton("❌ Annulla", "#757575", height=45)
        save_btn = ModernButton("💾 Salva Modifiche", "#2196F3", height=45)
    ...
    def setup_general_tab(self):
        ...
        title_label = QLabel("⚙️ Impostazioni lista")
        title_label.setStyleSheet("""
            QLabel {
                font-size: 16px;
                font-weight: bold;
        ...
        font_label = QLabel("Dimensione carattere:")
        font_label.setStyleSheet("font-size: 12px; font-weight: bold;")
        self.font_combo = QComboBox()
        self.font_combo.addItems(["piccolo", "medio", "grande"])
        current_font = self.settings.get('font_size', 'medio')
        ...
        self.strikethrough_check = QCheckBox("Mostra testo barrato per task completati")
        ...
        self.tab_widget.addTab(tab, "⚙️ Impostazioni")
    ...
    def setup_tasks_tab(self):
        ...
        add_btn = ModernButton("➕ Aggiungi Nuovo To-Do", "#4CAF50", height=45)
        add_btn.clicked.connect(self.add_new_task)
        layout.addWidget(add_btn)

        self.tab_widget.addTab(tab, "📋 Gestione To-Do")
    ...
    def refresh_tasks_layout(self):
        ...
        if not self.tasks:
            no_tasks_label = QLabel("Nessun To-Do presente. Clicca 'Aggiungi Nuovo To-Do' per iniziare!")
    ...
    def setup_file_tab(self):
        ...
        title_label = QLabel("💾 Gestione File")
        title_label.setStyleSheet("""
            QLabel {
                font-size: 16px;
                font-weight: bold;
        ...
        file_label = QLabel("File corrente:")
        file_label.setStyleSheet("font-size: 12px; font-weight: bold; color: #333333;")
        
        file_text = self.current_file_path if self.current_file_path else "Nessun file caricato"
        ...
        if self.current_file_path:
            unload_btn = ModernButton("🗑️ Rimuovi File Corrente", "#ff6b6b", height=45)
        ...
        self.tab_widget.addTab(tab, "💾 File e Salvataggio")
    ...
    def add_task_widget(self, index, task_data):
        ...
        link_label = QLabel("Collegamento (opzionale):")
        ...
        """)
        browse_btn = ModernButton("Sfoglia", "#E0E0E0", height=35)
        ...
        add_subtask_btn = ModernButton("➕ Aggiungi Task", "#E0E0E0", height=35)
        ...
        colors_title = QLabel("🎨 Personalizzazione Colori")
        ...
        base_color_label = QLabel("Colore titolo:")
        ...
        selected_color_label = QLabel("Colore completato:")
    ...
    def delete_task(self, index):
        reply = QMessageBox.question(
            self, "Elimina To-Do",
            f"Sei sicuro di voler eliminare il To-Do #{index + 1}?",
    ...
    def choose_color(self, index, color_type, button):
        ...
        color = QColorDialog.getColor(initial, self, f"Scegli colore {color_type}")
        if color.isValid():
    ...
    def unload_file(self):
        reply = QMessageBox.question(
            self, "Rimuovi File", 
            "Vuoi rimuovere il file corrente dalla memoria?\n\nLa lista corrente verrà svuotata.",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
        )
        if reply == QMessageBox.StandardButton.Yes:
    ...
    def reset_checks(self):
        QMessageBox.information(self, "Reset Check", "I check verranno resettati quando salverai le modifiche.")
    ...
    def load_configuration(self):
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Carica lista", "", "File di testo (*.txt);;Tutti i file (*.*)"
        )
        if file_path:
            try:
                tasks = []
                        ...
                        if line.startswith("[SETTINGS]"):
                            ...
                            while i < len(lines) and not lines[i].startswith("[TASK]") and lines[i].strip() != "":
                                ...
                                if line.startswith("title="):
                                    ...
                                    settings["font_size"] = line.split('=', 1)[1]
                ...
                QMessageBox.critical(self, "Errore", f"Impossibile caricare la lista: {e}")
    ...
    def save_as_configuration(self):
        file_path, _ = QFileDialog.getSaveFileName(
            self, "Salva lista", "", "File di testo (*.txt);;Tutti i file (*.*)"
        )
        if file_path:
    ...
    def _save_to_file(self, file_path):
        try:
            ...
            QMessageBox.critical(self, "Errore", f"Impossibile salvare la lista: {e}")
...
class InfoDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        desc_label = QLabel(
            "Questa applicazione ti aiuta nella gestione dei task:\n\n"
            "• Crea e personalizza le tue liste di attività\n"
            "• Organizza i To-Do con task e collegamenti\n"
            "• Salva e carica le tue liste in formato testo"
...
class TodoApp(QMainWindow):
    ...
    def refresh_tasks(self):
        ...
        personalize_btn = ModernButton("Personalizza", "#2196F3", height=40)
        ...
        font_size = font_sizes.get(self.settings.get('font_size', 'medio'), 14)
